 ...
 // Inlcuding the main libraries in this single file:
 ...
 */
 ...
 try {
   ...
   Ti.API.error("oAuth Error: " + e.message);
 }
 ...
 function OAuthAdapterNew(pConsumerSecret, pConsumerKey, pSignatureMethod, tokenFilename) {
   ...
   this.loadAccessToken = function (pService) {
     ...
     if (!persTokens) {
       ...
       Ti.API.error("could not find token in pers memory");
       return false;
     }
     Ti.API.info('Loading access token for service [' + pService + '].');

     Ti.API.info('Token contents [' + JSON.stringify(persTokens, null, 2) + '].');
     if (persTokens.accessToken === null || persTokens.accessTokenSecret === null) {
       accessToken = null;
       accessTokenSecret = null;
     ...
     if (persTokens.accessToken) {
       accessToken = persTokens.accessToken;
     } else {
       accessToken = null;
     }
     if (persTokens.accessTokenSecret) {
       accessTokenSecret = persTokens.accessTokenSecret;
     } else {
       accessTokenSecret = null;
     }
   ...
   // TODO - needs to be more generic than this. 3/06/2013
   ...
   this.getAccessToken = function (params) {
     ...
     function saveAccessToken(e) {
       ...
       // Ti.API.info('Object passed to save [' + JSON.stringify(this.accessTokens, null, 2) + '].');
     ...
     // Ti.API.info('Save Token Process --- START');
   ...
   this.authorizeUICallback = function (e) {
     ...
     if ('https://api.twitter.com/oauth/authorize' === loc) {
       ...
       setTimeout(function () {
         ...
         setTimeout(function () {
           // Ti.API.info('checking source code for PIN');
   ...
   this.setupWindow = function (childContent) {
     ...
     if (Ti.Platform.osname === 'iphone') {
       ...
       // Add android menu button for cancel here:
       ...
       activity.onCreateOptionsMenu = function (e) {
         ...
         menuItem.addEventListener("click", function (e) {
           ...
           window.close();
           window = null;
           pou.ui.twitter.active = null;
           pou.app.twitterAdmin.close();
         });
       }
   ...
   this.send = function (params) {
     ...
     client.onerror = function (e) {
       if (params.onError) {
         ...
         Ti.API.info(httpResponse);
         params.onError(httpResponse);
       }
     };
     client.onload = function () {
       Ti.API.info('*** HTTP connection onload ***');
       Ti.API.warn('*** sendStatus, Response: [' + client.status + '] ' + client.responseText);
     ...
     if (stickOAuthParam) { // Google Auth
       ...
       Ti.API.warn('Sending Headers');
       Ti.API.warn('Authorization: ' + header);
